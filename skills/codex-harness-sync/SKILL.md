...
2. Run `$PYTHON scripts/codex_harness.py sync --dry-run`. Explain that the command copies managed local files into the repository, including the config template; flag machine-specific paths, trust entries, commands, and runtime values for review.
...
5. Report changed paths and any values the user should remove before committing. Never copy credentials, session databases, chat history, caches, worktrees, or runtime binaries.
